...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!id || !getSubscriptionConfig(id, user.id)) return jsonNoStore({ error: "配置不存在" }, { status: 404 });
  ...
  try {
    return jsonNoStore({ ok: true, config: updateSubscriptionConfig(id, user.id, name), configs: listSubscriptionConfigs(user.id) });
  } catch {
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const id = parseConfigId((await context.params).id);
  if (!id || !getSubscriptionConfig(id, user.id)) return jsonNoStore({ error: "配置不存在" }, { status: 404 });
  if (listSubscriptionConfigs(user.id).length === 1) return jsonNoStore({ error: "至少保留一份配置" }, { status: 400 });

  deleteSubscriptionConfig(id, user.id);
  return jsonNoStore({ ok: true, configs: listSubscriptionConfigs(user.id) });
}
