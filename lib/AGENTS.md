...
# Ownership
- `db.ts` owns schema initialization, subscription-configuration isolation, and persistence helpers.
...
- Subscription configuration data is scoped by configuration ID; preserve the legacy default configuration and its `SUB_TOKEN` URL during migrations.
- Legacy schema migration must first create `app.db.pre-multi-config.bak`, then apply all schema and data changes in one SQLite transaction.
